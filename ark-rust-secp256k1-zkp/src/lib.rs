...
//! (as long as you are dependening on the correct version) which means [`SecretKey`]s and the
//! [`Context`] are interoperable.
...
pub enum Error {
    ...
    /// Failed to decrypt an adaptor signature because of an internal error within
    /// `libsecp256k1-zkp`
    CannotDecryptAdaptorSignature,
    /// Failed to recover an adaptor secret from an adaptor signature because of an internal error
    /// within `libsecp256k1-zkp`
    CannotRecoverAdaptorSecret,
    /// Given adaptor signature is not valid for the provided combination of public key, encryption
    /// key and message
